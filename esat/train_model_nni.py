...
def save_checkpoint(state, is_best, dst_path='log/', filename='ckpt.pth.tar'):
...
def get_params():
    ...
    parser.add_argument("--hidden_dim", type=int, default=64, metavar='MO')
    parser.add_argument('--lr', type=float, default=0.00001, metavar='LR',
                        help='learning rate (default: 0.01)')
    parser.add_argument('--weight_decay1', type=float, default=0.001, metavar='M',
                        help='L1 weight_decay')
    parser.add_argument('--weight_decay2', type=float, default=0.01, metavar='M1',
                        help='mim weight_decay')
    parser.add_argument('--epoch', type=int, default=400, metavar='N',
...
def main(args):
    ...
    for epoch in range(args["epoch"]):
        ...
        for step, (feat, surv_time, status) in enumerate(tqdm(train_loader)):
            ...
            theta_conf = conf.reshape(-1)
            exp_theta_conf = torch.exp(-theta_conf)
            exp_theta_pred = torch.exp(theta_pred)
            loss_sur = -torch.mean(exp_theta_conf*(theta_pred - torch.log(torch.sum(exp_theta_pred * R_matrix, dim=1))) * y_status)
            loss_mim = torch.var(theta_pred * R1_matrix)-torch.var(theta_pred* R2_matrix)
            loss_contra= torch.var(theta_pred* R2_matrix)
            loss_conf =  torch.sum(theta_conf)
            l1_norm = 0.
            for W in model.parameters():
                l1_norm += torch.abs(W).sum()
            loss = loss_sur + args["weight_decay1"] * l1_norm +0.1*loss_conf
            print(loss_sur, args["weight_decay1"] * l1_norm , args["weight_decay2"] * loss_contra,0.1*loss_conf)
            loss.backward()
            optimizer.step()
...
#test
        ...
        try:
            ...
            c_index_test = -1
        nni.report_intermediate_result(c_index_test)
    ...
    nni.report_final_result(best_cindex_val)
